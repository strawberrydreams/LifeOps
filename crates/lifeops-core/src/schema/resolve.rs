...
use crate::schema::raw::{load_raw_dir, RawFieldDef, RawSchema};
...
impl SchemaSet {
    ...
    pub fn from_raw(
        ...
        for name in raw.keys() {
            ...
            schemas.insert(
                name.clone(),
                ResolvedSchema { name: name.clone(), extends: leaf.extends.clone(), fields },
            );
        }
...
fn ancestor_chain(
    ...
    while let Some(parent) = raw[&cur].0.extends.clone() {
        ...
        if chain.contains(&parent) {
            chain.push(parent);
            return Err(SchemaError::Cycle { chain: chain.join(" → ") });
        }
...
fn convert_field(ty: &str, field: &str, def: &RawFieldDef) -> Result<ResolvedField, SchemaError> {
    ...
    if is_enum && def.options.is_none() {
        return Err(SchemaError::EnumWithoutOptions { ty: ty.to_string(), field: field.to_string() });
    }
    if kind.contains_ref() && def.target.is_none() {
        return Err(SchemaError::RefWithoutTarget { ty: ty.to_string(), field: field.to_string() });
    }
    Ok(ResolvedField {
        kind,
...
mod tests {
    ...
    fn field_order로_재배치하고_누락_필드는_뒤에_붙는다() {
        ...
        .unwrap();
        let names: Vec<&str> = set.get("시계").unwrap().fields.keys().map(|s| s.as_str()).collect();
    ...
    fn 잘못된_kind와_제약_누락은_에러() {
        ...
        .unwrap();
        assert_eq!(set.get("A").unwrap().fields["x"].kind, FieldKind::List(Box::new(FieldKind::Ref)));
    }
}
